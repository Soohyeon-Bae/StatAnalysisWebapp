...
def build_regression_interpretation(result, dependent: str, family: str, alpha: float = 0.05) -> str:
    ...
    for row in sig_rows:
        ...
        if odds_col:
            bits.append(f"{row['term']}의 오즈비는 {_fmt_num(row[odds_col])}로, 해당 변수가 증가할수록 {dependent}=1의 가능성이 {direction}하는 방향이었다 ({_p_text(row[p_col])})")
    main = "; ".join(bits) if bits else "선택한 유의수준에서 뚜렷하게 유의한 설명변수가 많지 않았다"
    return f"{dependent}을 종속변수로 한 로지스틱 회귀분석을 실시하였다. 모형 적합도 지표인 McFadden pseudo R²는 {_fmt_num(pseudo_r2)}였다. {main}. 그러나 이 역시 관측자료 기반의 연관 추정이므로 추가적인 연구설계 없이는 인과효과로 단정할 수 없다."
...
def build_event_study_interpretation(result, alpha: float = 0.05) -> str:
    ...
    return (
        f"Event study는 기준시점 -1을 준거로 상대시점별 효과를 추정한 결과이다. 사전기간에서 유의한 계수는 {pre_sig}개, 사후기간에서 유의한 계수는 {post_sig}개였다. "
...
def build_survival_interpretation(result, alpha: float = 0.05) -> str:
    ...
    txt += " 다만 검열(censoring)이 비정보적이라는 가정과 비례위험 가정이 적절한지 추가 점검이 필요하다."
    return txt
