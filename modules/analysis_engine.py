...
def _prepare_series(df: pd.DataFrame, col: str, col_type: str) -> pd.DataFrame:
    # 공통 전처리는 "정리"만 담당. 분석 의미를 바꾸는 인코딩은 하지 않음.
...
def run_regression(
    ...
    encoding_info = None
    if family == "logistic":
        encoded_y, encoding_info = _encode_binary_for_regression(work[dependent])
        work[dependent] = encoded_y
    ...
    if family == "linear":
        ...
        summary = {
            ...
            "encoding_info": str(encoding_info) if encoding_info is not None else None,
        }
        narrative = (
    ...
    details = {"model_data_preview": work.head(20)}
    if encoding_info is not None:
        details["dependent_encoding"] = pd.DataFrame(
            [{"original_value": k, "encoded_value": v} for k, v in encoding_info.items()]
        )
...
def _parallel_trend_test(event_table: pd.DataFrame) -> dict[str, float]:
    ...
    df_wald = len(valid)
    joint_p = 1 - chi2.cdf(wald, df_wald)
    sig_pre = int((pre["p_value"] < 0.05).fillna(False).sum())
    return {
        "pretrend_joint_p_value": float(joint_p),
        "num_significant_pretrends": sig_pre,
    }
...
def run_did_analysis(
    ...
    work["treated"] = (
        df[treatment].astype("string").str.strip() == str(treated_value).strip()
    ).astype(float)
...
def run_event_study_analysis(
    ...
    if not ((work["rel_period"] == -1).any() and (work["rel_period"] >= 0).any()):
        raise AnalysisError("기준시점 -1과 사후 기간이 모두 필요합니다.")
    ...
    summary = {
        ...
        "n": int(model.nobs),
        "max_abs_pretrend": float(pre_table["coef"].abs().max()) if not pre_table.empty else np.nan,
        "r_squared": float(model.rsquared),
        "pretrend_joint_p_value": parallel_info["pretrend_joint_p_value"],
        "num_significant_pretrends": parallel_info["num_significant_pretrends"],
    }
    narrative = (
    ...
    detail_tables = {
        ...
        "coefficient_table": coef,
        "time_mapping": pd.DataFrame({"original_time": list(time_map.keys()), "time_index": list(time_map.values())}),
        "parallel_trend_test": pd.DataFrame([parallel_info]),
    }
    ...
    if group_var:
        ...
        for g in groups:
            ...
            try:
                ...
                pass
            group_summary_rows.append({"group": str(g), "n": int(len(gdf)), "events": int(gdf["event_observed"].sum()), "median_survival_time": median_surv})
        if len(groups) == 2:
        ...
        try:
            ...
            pass
        group_summary_rows.append({"group": "All", "n": int(len(work)), "events": int(work["event_observed"].sum()), "median_survival_time": median_surv})
        lr_stat = np.nan
        lr_p = np.nan

    km_table = pd.concat(km_rows, ignore_index=True)
    group_summary = pd.DataFrame(group_summary_rows)
    detail_tables: Dict[str, pd.DataFrame] = {"kaplan_meier_curve": km_table, "group_summary": group_summary}
...
def _run_categorical_association(work: pd.DataFrame, x: str, y: str, alpha: float, method_override: Optional[str]) -> AnalysisResult:
    ...
    stats_table = pd.DataFrame([{"x": x, "y": y, "test": test, "statistic": stat, "p_value": p, "effect_size": eff, "n": int(table.to_numpy().sum())}])
