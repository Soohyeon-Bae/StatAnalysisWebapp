...
def _sync_profile_table(df: pd.DataFrame):
    ...
    for col in profile_df["column"].tolist():
        if str(col).startswith("policy_after"):
            profile_df.loc[profile_df["column"] == col, "user_type"] = "binary"
        if str(col).startswith("duration_from_"):
            profile_df.loc[profile_df["column"] == col, "user_type"] = "continuous"
...
with tab_basic:
    ...
    if st.button("기초분석 실행", type="primary"):
        ...
        _result_block("기초분석 결과", build_bivariate_interpretation(result, x, y, x_type, y_type, alpha=alpha), result.stats_table, result.detail_tables)
...
with tab_reg:
    ...
    if st.button("회귀분석 실행", type="primary"):
        ...
        _display_summary_cards(result.summary)
        st.info(result.narrative)
        _result_block("회귀분석 결과", build_regression_interpretation(result, dependent, family), result.stats_table, result.detail_tables)
...
with tab_causal:
    ...
    if dt_or_num_candidates:
        ...
        if st.button("Before/After 변수 생성", key="make_post"):
            ...
            st.success(f"{new_post_name} 생성 완료. 아래 Time variable에서 선택할 수 있습니다.")
            st.rerun()
    ...
    if st.button("DiD 실행", type="primary"):
        try:
            result = run_did_analysis(df=df, outcome=outcome, treatment=treatment, time_var=time_var, type_map=type_map, treated_value=treated_value, intervention_value=intervention_value, controls=did_controls, unit_var=None if unit_var == "(없음)" else unit_var, add_unit_fe=add_unit_fe, add_time_fe=add_time_fe)
        except Exception as e:
            st.error(str(e))
            st.stop()
        _display_summary_cards(result.summary)
        st.info(result.narrative)
        _result_block("DiD 결과", build_did_interpretation(result, outcome), result.stats_table, result.detail_tables)
    ...
    if st.button("Event Study 실행", type="primary"):
        try:
            result = run_event_study_analysis(df=df, outcome=es_outcome, treatment=es_treatment, time_var=es_time, type_map=type_map, treated_value=es_treated_value, intervention_value=es_intervention, controls=es_controls, unit_var=None if es_unit == "(없음)" else es_unit, add_unit_fe=es_unit_fe, window_pre=int(pre_w), window_post=int(post_w))
        except Exception as e:
            st.error(str(e))
            st.stop()
        _display_summary_cards(result.summary)
        st.info(result.narrative)
        _result_block("Event Study 결과", build_event_study_interpretation(result), result.stats_table, result.detail_tables)
...
with tab_survival:
    ...
    if datetime_candidates:
        ...
        if st.button("duration 변수 생성", key="make_duration"):
            base_df = st.session_state["df"].copy()
            new_df = _add_duration_from_datetime(base_df, duration_base, ref_date, duration_unit, new_duration_name)
            _update_session_df(new_df)
            st.session_state["last_generated_var"] = new_duration_name
            st.success(f"{new_duration_name} 생성 완료. 아래 Duration variable에서 선택할 수 있습니다.")
            st.rerun()
    ...
    if st.button("생존분석 실행", type="primary"):
        try:
            result = run_survival_analysis(df=df, duration_var=duration_var, event_var=event_var, type_map=type_map, event_value=event_value, group_var=None if group_var == "(없음)" else group_var, group_values=selected_groups, covariates=covariates)
        except Exception as e:
            st.error(str(e))
            st.stop()
        _display_summary_cards(result.summary)
        st.info(result.narrative)
        _result_block("생존분석 결과", build_survival_interpretation(result), result.stats_table, result.detail_tables)
...
with tab_advanced:
    ...
    if st.button("전체 변수 탐색 실행", type="primary"):
        ...
        _display_summary_cards(result.summary)
        st.info(result.narrative)
        _result_block("고급탐색 결과", build_bulk_interpretation(result, alpha=alpha), result.stats_table, None)
