...
def _uploaded_signature(uploaded, sheet_name):
    if uploaded is None:
        return None
    return (uploaded.name, getattr(uploaded, "size", None), sheet_name)
...
def _sync_profile_table(df: pd.DataFrame):
    ...
    # 생성형 변수들에 대한 기본 타입 보정
    if "column" in profile_df.columns:
        mask_policy = profile_df["column"].astype(str).str.contains("policy_after", case=False, na=False)
        profile_df.loc[mask_policy, "user_type"] = "binary"

        mask_duration = profile_df["column"].astype(str).str.contains("duration_", case=False, na=False)
        profile_df.loc[mask_duration, "user_type"] = "continuous"
...
with tab_data:
    ...
    if uploaded is not None:
        ...
        if uploaded.name.lower().endswith(".xlsx"):
            ...
            prev_sheet = st.session_state.get("selected_sheet")
            default_idx = sheets.index(prev_sheet) if prev_sheet in sheets else 0
            selected_sheet = st.selectbox("시트 선택", sheets, index=default_idx, key="sheet_selector")
        else:
            selected_sheet = None

        current_sig = _uploaded_signature(uploaded, selected_sheet)
        prev_sig = st.session_state.get("uploaded_signature")

        # 파일이 처음 업로드되었거나, 파일/시트가 바뀌었을 때만 원본 다시 로드
        if ("df" not in st.session_state) or (prev_sig != current_sig):
            try:
                loaded_df, active_sheet = load_uploaded_file(uploaded, selected_sheet)
            ...
            _update_session_df(loaded_df)
            st.success(f"파일 로드 완료: {uploaded.name} / sheet={active_sheet} / rows={len(loaded_df):,} / cols={len(loaded_df.columns):,}")
        else:
            active_sheet = st.session_state.get("selected_sheet")
            st.info(f"현재 세션 데이터 사용 중: {st.session_state.get('uploaded_file_name', uploaded.name)} / sheet={active_sheet}")

        current_df = st.session_state["df"]
        st.dataframe(current_df.head(20), use_container_width=True)

        generated_cols = [c for c in current_df.columns if "policy_after" in str(c).lower() or "duration_" in str(c).lower()]
        if generated_cols:
            st.success(f"생성된 변수: {', '.join(map(str, generated_cols))}")
...
with tab_reg:
    ...
    family = st.selectbox(
        "회귀 유형",
        [default_family, "linear", "logistic"] if default_family == "logistic" else [default_family, "logistic"],
        index=0,
        key="reg_family",
    )
...
with tab_causal:
    ...
    did_controls = st.multiselect(
        "통제변수",
        [c for c in eligible_outcomes + eligible_treatments if c not in {outcome, treatment, time_var, unit_var}],
        key="did_controls",
        format_func=fmt,
    )
    ...
    es_controls = st.multiselect(
        "통제변수",
        [c for c in eligible_outcomes + eligible_treatments if c not in {es_outcome, es_treatment, es_time, es_unit}],
        key="es_controls",
        format_func=fmt,
    )
...
with tab_survival:
    ...
    covariates = st.multiselect(
        "Cox 공변량(선택)",
        [c for c in cov_candidates if c not in {duration_var, event_var, group_var}],
        key="surv_covs",
        format_func=fmt,
    )
